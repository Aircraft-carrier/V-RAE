...
Environment:
  ...
  UCF_DRY_RUN       Set to 1 to validate routing and print the table without launching.
  UCF_BACKGROUND    Set to 1 to launch with nohup and write one log per node.
  UCF_LOG_DIR       Background log directory (default: outputs/logs).
  UCF_PYTHON        Optional Python executable override.
  UCF_TORCHRUN      Optional torchrun executable override.
...
Examples:
  ...
  MASTER_ADDR=10.0.0.8 scripts/train/ucf101.sh 2 8 0 vjepa
  MASTER_ADDR=10.0.0.8 scripts/train/ucf101.sh 2 8 1 vjepa

  # Validate routing without starting workers.
  UCF_DRY_RUN=1 scripts/train/ucf101.sh 1 8 0 eupe --max-steps 1000
EOF
}
...
config_path="${project_root}/configs/training/ucf_videogen/${config_name}"
...
if [[ ! -f "${config_path}" ]]; then
  echo "UCF101 VideoGen config does not exist: ${config_path}" >&2
  exit 2
fi
...
python_bin="${UCF_PYTHON:-$(command -v python)}"
torchrun_bin="${UCF_TORCHRUN:-$(command -v torchrun)}"
...
export TORCHINDUCTOR_CACHE_DIR="${TORCHINDUCTOR_CACHE_DIR:-${cache_root}/vrae_ucf_${encoder_name}_inductor_cache}"
...
from vrae.training.ucf_videogen.train import validate_build
...
launch_args=(
  ...
  -m vrae.training.ucf_videogen.train
  --config "${config_path}"
)
...
try:
    ...
    print("\nUCF101 VideoGen training configuration")
    previous = None
...
table = Table(
    title="UCF101 VideoGen Training Configuration",
...
if [[ "${UCF_DRY_RUN:-0}" == "1" ]]; then
  exit 0
fi

if [[ "${UCF_BACKGROUND:-0}" == "1" ]]; then
  log_dir="${UCF_LOG_DIR:-${project_root}/outputs/logs}"
  log_path="${UCF_LOG_PATH:-${log_dir}/${run_name}_node${node_rank}_$(date -u +%Y%m%dT%H%M%SZ).log}"
