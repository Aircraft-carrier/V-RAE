...
usage() {
  cat <<'EOF'
Usage: scripts/eval/ucf101_rfvd.sh <gpu_count> <vrae> <global_batch_size> [eval_args...]
...
Examples:
  ...
  CUDA_VISIBLE_DEVICES=2,3 scripts/eval/ucf101_rfvd.sh 2 siglip 128 --smoke
EOF
}
...
base_config="${project_root}/configs/evaluation/ucf101_rfvd/${config_name}"
...
routed_config="$(mktemp -t ucf101-rfvd-config.XXXXXX.yaml)"
...
echo "Launching UCF101 rFVD: vrae=${vrae}, config=${config_name}, GPUs=${gpu_count}, global_batch_size=${global_batch_size}" >&2
