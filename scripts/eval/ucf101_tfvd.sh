...
usage() {
  cat <<'EOF'
Usage: scripts/eval/ucf101_tfvd.sh <gpu_count> <vrae> <global_batch_size> [eval_args...]
...
Examples:
  ...
  scripts/eval/ucf101_tfvd.sh 8 vjepa 512 --resume
  CUDA_VISIBLE_DEVICES=2,3 scripts/eval/ucf101_tfvd.sh 2 eupe 128 --smoke
EOF
}
...
base_config="${project_root}/configs/evaluation/ucf101_tfvd/${config_name}"

required_files=(
  "${base_config}"
  "${project_root}/data/metadata/ucf101_test_split1_tfvd_f24_dt3.json"
  "${project_root}/ckpts/vrae/${checkpoint_name}"
  "${project_root}/ckpts/eval_models/i3d_torchscript.pt"
)
for required_file in "${required_files[@]}"; do
  if [[ ! -f "${required_file}" ]]; then
    echo "required UCF101 tFVD file does not exist: ${required_file}" >&2
    exit 1
  fi
done

routed_config="$(mktemp -t ucf101-tfvd-config.XXXXXX.yaml)"
...
echo "Launching UCF101 tFVD: vrae=${vrae}, config=${config_name}, GPUs=${gpu_count}, global_batch_size=${global_batch_size}" >&2

cd -- "${project_root}"
export PYTHONPATH="${project_root}/src${PYTHONPATH:+:${PYTHONPATH}}"
torchrun --standalone --nproc_per_node="${gpu_count}" \
  -m vrae.evaluation.ucf101_tfvd.run \
