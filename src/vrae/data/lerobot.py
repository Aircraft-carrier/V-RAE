...
except ImportError:  # Keep importing vrae.data possible outside the LeRobot environment.
    class LeRobotDataset:  # type: ignore[no-redef]
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            raise ImportError("LeRobotVideoDataset requires the lerobot v3.0 environment")
...
class LeRobotVideoDataset(LeRobotDataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.null_task_id = int(getattr(self.meta, "total_tasks", 0) or 0)
        ...
        for row in columns:
            ...
            if stop - start < self.sampler.required_frames:
                continue
            task_index = self.meta.get_task_index(row["tasks"][0])
            self._episodes.append(
                (
                    int(row["episode_index"]),
                    start,
                    stop,
                    self.null_task_id if task_index is None else int(task_index),
                )
            )
    ...
    def _resolve_camera_keys(
        ...
        for position, value in enumerate(values):
            ...
            dtype = feature.get("dtype") if isinstance(feature, Mapping) else getattr(feature, "dtype", None)
            if feature is None or str(dtype) != "image" or shape is None or len(shape) != 3 or int(shape[-1]) != 3:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if self.image_size is not None and tuple(video.shape[-2:]) != (self.image_size, self.image_size):
            video = torch.nn.functional.interpolate(
                video.flatten(0, 1), size=(self.image_size, self.image_size),
