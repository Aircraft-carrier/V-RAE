...
def ensure_latent_stats(config: Mapping[str, Any], paths: ProjectPaths) -> Path:
    """Create the configured K600 latent normalizer before creating the training run."""
    ...
    compute_class_conditional_latent_stats(config, paths, dataset_name="k600")
...
def validate_build(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if config["task"] != "k600_videogen":
        raise ValueError("K600 entry requires task=k600_videogen")
    if config["data"]["dataset"] != "k600" or config["data"]["split"] != "train":
        raise ValueError("K600 generation trains on the K600 train split")
    if not config["data"].get("manifest"):
        raise ValueError("K600 requires a labeled manifest; volume directories are not classes")
    if int(config["dit"]["num_classes"]) != 600:
        raise ValueError("K600 generation requires 600 classes")
    if int(config["data"]["num_frames"]) not in {16, 20}:
        raise ValueError("K600 supports the formal 16/20-frame protocols")
    runtime = config.get("runtime", {})
    pipeline = runtime.get("data_pipeline", {})
    if pipeline.get("kind", "torchcodec_cpu_bounded") != "torchcodec_cpu_bounded":
        raise ValueError("K600 training requires the bounded TorchCodec CPU pipeline")
    ...
    if transport["prediction"] != "x" or abs(transport["time_dist_shift"] - expected_shift) > 1e-9:
        raise ValueError("K600 requires x-prediction and the latent-dimension time shift")
    return {
        "task": "k600_videogen",
        "dit": "vrae_video_dit",
        "num_classes": 600,
...
def main() -> None:
    arguments = parse_config_argument("Train K600 V-RAE VideoDiT")
    ...
    train_class_conditional(config, paths, run, dataset_name="k600", max_steps=arguments.max_steps)
